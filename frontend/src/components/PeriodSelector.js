...
export default function PeriodSelector({ onSelect, onBack, title, description }) {
  ...
  if (loading) return (
    <div className="theme-page game-screen min-h-screen flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-sm font-semibold game-text-muted">Đang tải danh sách triều đại...</p>
      </div>
    </div>
  );

  return (
    <div className="theme-page game-screen min-h-screen p-4 sm:p-8 flex flex-col items-center">
      <div className="max-w-3xl w-full">
        {/* Header */}
        <div className="text-center mb-8">
          <p className="text-xs font-bold uppercase tracking-[0.2em] mb-2" style={{ color: 'var(--field-label)' }}>⚔️ Chọn Thời Kỳ</p>
          ...
          <p className="text-sm game-text-muted">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-8 stagger-children">
          ...
          {lessons.map((lesson, idx) => (
            <div 
              ...
              className="p-5 rounded-2xl cursor-pointer transition-all hover:scale-[1.02] active:scale-[0.98] flex flex-col items-center justify-center text-center animate-fade-in-up opacity-0"
              style={{ background: 'var(--game-surface-soft)', border: '1px solid var(--game-border)', animationDelay: `${(idx + 1) * 0.05}s`, animationFillMode: 'forwards' }}
            >
              <span className="text-3xl mb-2">📜</span>
              <h3 className="text-sm font-bold leading-tight" style={{ color: 'var(--game-text)' }}>{lesson.title}</h3>
              <p className="text-[11px] mt-1 line-clamp-2 game-text-muted">{lesson.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="text-center">
          <button 
            ...
            style={{ background: 'var(--game-surface-subtle)', border: '1px solid var(--game-border)', color: 'var(--game-text-secondary)' }}
          >
